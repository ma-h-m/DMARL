...
def train(epochs=10, agent_info_list=None, batch_size=4096):
    ...
    for i in range(epochs):
        ...
        for agent_info in agent_info_list:
            ...
            # Step 1: Save old model parameters (before training)
            old_params = {
                'actor': {name: param.detach().cpu().clone() for name, param in policy_instance.model.actor.named_parameters()},
                'critic': {name: param.detach().cpu().clone() for name, param in policy_instance.model.critic.named_parameters()}
            }

            # Step 2: Train the policy
            gradients_this_epoch = policy_instance.train(batch)
            
            # Step 3: Calculate parameter differences (pseudo-gradients)
            new_params = {
                'actor': {name: param.detach().cpu() for name, param in policy_instance.model.actor.named_parameters()},
                'critic': {name: param.detach().cpu() for name, param in policy_instance.model.critic.named_parameters()}
            }

            # Calculate the difference between old and new parameters to get pseudo-gradients
            pseudo_gradients = {}
            for model_type in ['actor', 'critic']:
                model_gradients = {}
                for name in old_params[model_type]:
                    model_gradients[name] = new_params[model_type][name] - old_params[model_type][name]
                pseudo_gradients[model_type] = model_gradients

            # Step 4: Accumulate pseudo-gradients
            if agent_info["agent_id"] not in all_gradients:
                all_gradients[agent_info["agent_id"]] = {
                    'actor': {key: value.clone() for key, value in pseudo_gradients['actor'].items()},
                    'critic': {key: value.clone() for key, value in pseudo_gradients['critic'].items()}
                }
            else:
                for model_type in ['actor', 'critic']:
                    for key in all_gradients[agent_info["agent_id"]][model_type]:
                        all_gradients[agent_info["agent_id"]][model_type][key] += pseudo_gradients[model_type][key]

            # Step 5: Compute average reward for logging
